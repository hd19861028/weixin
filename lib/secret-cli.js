...
function RSA2_Create(params) {
	...
	signer.update(buff);
	var privateKey = fs.readFileSync(path.join(process.env.HOME, '/.ssh/', 'id_rsa')).toString();
...
function RSA2_Valid(params, sign) {
	...
	var buff = new Buffer(params, 'utf-8');
	var publicKey = fs.readFileSync(path.join(process.env.HOME, '/.ssh/', 'id_rsa.pub')).toString();
...
2. 利用私钥生成签名串
	secret -m=rsa2 -str=123456
3. 验签
	secret -m=rsa2 -str=123456 -pri=第二步返回的签名结果
