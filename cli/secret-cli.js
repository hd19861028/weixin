...
for (var k in argMapping) {
	...
	switch (k) {
		case "-h":
			var help = {
				"-m": "\t\t加密方式: md5,sha1,sha256,sha512,rsa2",
...
`
		...
		case "-m":
			...
			break;
		case "-rsapri":
			rsa_pri = path.resolve(os.homedir(), v);
			break;
		case "-rsapub":
			rsa_pub = path.resolve(os.homedir(), v);
			break;
		case "-d":
			aes_de = true;
			break;
...
function RSA2_Create(params) {
	let signer = crypto.createSign('RSA-SHA256');
	var buff = new Buffer(params, 'utf-8');
	signer.update(buff);
	let rsa = rsa_pri || ""
	if (!rsa_pri) rsa = path.join(os.homedir(), '/.ssh/', 'id_rsa');
	var privateKey = fs.readFileSync(rsa).toString();
	let sign = signer.sign(privateKey, 'base64');
	return sign;
}

function RSA2_Valid(params, sign) {
	let verify = crypto.createVerify('RSA-SHA256');
	var buff = new Buffer(params, 'utf-8');
	let rsa = rsa_pub || ""
	if (!rsa_pri) rsa = path.join(os.homedir(), '/.ssh/', 'id_rsa.pub');
	var publicKey = fs.readFileSync(rsa).toString();
	return verify.update(buff).verify(publicKey, sign, 'base64');
}
...
if (method) {
	if (method == "rsa2") {
		if (pri) {
			console.log("RSA2验签")
			console.log(RSA2_Valid(str, pri))
		} else {
			console.log("RSA2签名")
			console.log(RSA2_Create(str))
		}
	} else
